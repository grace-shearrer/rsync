...
#    df = pd.read_table(files)###<pandas.io.excel.ExcelFile object at 0x105a31e10>####
#    data=df.reset_index().groupby(['Participant ID']).aggregate(np.sum)
#    #daya2=data.groupby(['Participant ID']).aggregate(np.sum)
#    #data4=daya2.aggregate(np.sum)
#    print(data.head())
#    data.to_csv(files+'.csv', sep=",")
...
for files in allFiles:
    ...
    #data1=df.parse(files, index_col=0)
    df.reset_index()
    daya4=df.groupby(['Participant ID'])
    data5=daya4.aggregate(np.sum)
    data5.to_csv(files+'.csv', sep=",")
    
#data=pd.ExcelFile('totals_nutrients.xls')
#
#data.sheet_names
#
#data1=data.parse('totals_nutrients.xls', index_col=0)
#
...
#data5=daya4.aggregate(np.sum)
#
#data5.to_csv('final2.csv', sep=",")

#data6 = data5.ix[3:]
#
#data6.describe()#this works on the python object
#
#data6.to_csv('final3.csv', sep=",")
#
#print(ro.r('fame <-read.table("final2.csv", sep=",", header=T)'))
#
...
#
#ro.r('write.table(fame, "calculated_nutrients.csv", sep=",", row.names=FALSE)')
